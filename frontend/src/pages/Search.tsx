    ...
    useEffect(() => {
        ...
        if (listData && Array.isArray(listData)) {
            const mappedResults = listData.map((item: any) => ({
                ...
                has_substitutes: item.has_substitutes
            }));
        ...
        <div className="flex flex-col items-center min-h-[calc(100vh-80px)] px-4 bg-gradient-to-b from-white to-blue-50 dark:from-gray-950 dark:to-gray-900 py-10">
            <div className={`w-full max-w-3xl text-center space-y-8 animate-in fade-in zoom-in duration-500 transition-all ${triggeredQuery ? 'mb-8' : 'mb-0 min-h-[60vh] flex flex-col justify-center'}`}>
                <div className="space-y-4">
                    <h1 className="text-4xl md:text-6xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-brand-navy to-brand-teal dark:from-white dark:to-brand-teal tracking-tight animate-in slide-in-from-bottom-5 duration-500">
                        Smart Medicine Search
                    </h1>
                    <p className="text-lg md:text-xl text-gray-600 dark:text-gray-300 max-w-2xl mx-auto animate-in slide-in-from-bottom-5 duration-700 delay-100">
                        AI-powered insights, price comparisons, and substitute recommendations. We help you make informed healthcare decisions.
                    </p>
                </div>
            ...
            {/* Results Section */}
            {triggeredQuery && (
                <div className="w-full max-w-7xl animate-in slide-in-from-bottom-10 fade-in duration-700">
                    ...
                    {isLoading ? (
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 pb-10 items-start">
                            {/* Left Column: Database Results */}
                            <div className="space-y-4">
                                <div className="sticky top-24 z-10 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md p-4 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm mb-4">
                                    <h3 className="text-lg font-bold text-gray-800 dark:text-white flex items-center gap-2">
                                        <Pill className="w-5 h-5 text-brand-blue" />
                                        Verified Database Matches
                                        <span className="text-xs font-normal text-gray-500 ml-auto bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded-full">{dbResults.length} records</span>
                                    </h3>
                                </div>
                                {dbResults.length > 0 ? (
                                    <div className="grid gap-4">
                                        {dbResults.map((medicine, index) => (
                                            <div
                                                key={medicine.id}
                                                className="bg-white dark:bg-gray-800 rounded-xl p-6 border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-all duration-300 animate-in slide-in-from-bottom-5 fill-mode-backwards"
                                                style={{ animationDelay: `${index * 100}ms` }}
                                            >
                                                <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                                                    <div className="flex items-start gap-4">
                                                        <div className="w-12 h-12 rounded-full bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center text-brand-blue flex-shrink-0 transition-transform group-hover:scale-110">
                                    ...
                                    <div className="text-center py-12 px-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-700">
                                        <p className="text-gray-500">No exact matches found in our local database.</p>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="space-y-4">
                                <div className="sticky top-24 z-10 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md p-4 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm mb-4">
                                    <h3 className="text-lg font-bold text-gray-800 dark:text-white flex items-center gap-2">
                                        ...
                                        AI Insights
                                        <span className="text-xs font-normal text-gray-500 ml-auto bg-purple-50 dark:bg-purple-900/20 text-purple-600 px-2 py-0.5 rounded-full">GenMedAI</span>
                                    </h3>
                                </div>

                                {aiResults.length > 0 ? (
                                    <div className="space-y-6">
                                        {aiResults.map((medicine, index) => (
                                            <div
                                                key={medicine.id}
                                                className="animate-in slide-in-from-bottom-5 fade-in fill-mode-backwards duration-700"
                                                style={{ animationDelay: `${index * 150}ms` }}
                                            >
                                                {/* Chat Bubble Style Explanation */}
                                                {medicine.explanation && (
                                                    <div className="flex gap-4 mb-6 items-start">
                                                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-brand-teal to-blue-600 flex items-center justify-center flex-shrink-0 shadow-lg shadow-brand-teal/20 text-white">
                                                            <Bot className="w-6 h-6" />
                                                        </div>
                                                        <div className="bg-white dark:bg-gray-800 rounded-2xl rounded-tl-none p-6 shadow-sm border border-gray-100 dark:border-gray-700 relative flex-grow group hover:border-brand-teal/30 transition-colors">
                                    ...
                                    <div className="text-center py-12 px-4 bg-gray-50 dark:bg-gray-800/50 rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-700">
                                        <p className="text-gray-500">No AI insights generated for this query.</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="text-center py-20 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-800">
                            ...
                            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">No verified information found</h3>
                            <p className="text-gray-500 dark:text-gray-400 max-w-md mx-auto">
                                We couldn't find any medicine records matching "{triggeredQuery}". Please check the spelling.
                            </p>
                        </div>
                    )}
                    ...
                    <div className="mt-10 pt-6 border-t border-gray-100 dark:border-gray-800 text-center">
                        <p className="text-xs text-gray-400 dark:text-gray-500">
                            Disclaimer: GenMedAI is an information platform and does not sell medicines. All purchasing links redirect to verified third-party pharmacies.
                            Prices and availability are subject to change by the respective sellers. Always verify details on the partner site.
                        </p>
                    </div>
                </div>
            )}
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] overflow-y-auto border border-gray-200 dark:border-gray-700 flex flex-col">
                        <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center sticky top-0 bg-white dark:bg-gray-900 z-10">
                            <div>
                                <h2 className="text-xl font-bold text-gray-900 dark:text-white">Alternatives for {selectedMedicine.name}</h2>
                                <p className="text-sm text-gray-500">Medicines with the same salt composition.</p>
                            </div>
                            <button
                        ...
                        <div className="p-6 space-y-4">
                            {isLoadingSubstitutes ? (
                                ...
                                <div className="text-center py-10">
                                    <p className="text-gray-500">No matching alternatives found.</p>
                                </div>
                            )}
                        </div>
                    </div>
